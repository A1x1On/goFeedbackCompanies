...
func ProxyRequest(httpType string, urlPath string, params map[string]string) []byte {
	...
	helper.IfError(err, "Can't (url.Parse) passed val from [config.Set.Proxy.Adress]")

	//creating the URL to be loaded through the proxy
	urlStr    	  := urlPath
	url, err  	  := url.Parse(urlStr)
	helper.IfError(err, "Can't (url.Parse) passed val [urlPath string]")
	...
	if params != nil {
		...
		helper.IfError(err, "Can't (json.Marshal) passed val [params map[string]string]")
	...
	helper.IfError(err, "Can't generate the HTTP request (http.NewRequest)")
	...
	helper.IfError(err, "Can't print the request to the console (httputil.DumpRequest)")
	log.Println(string(dump))

	//calling the URL
	response, err := client.Do(request)
	helper.IfError(err, "Can't call the URL (client.Do)")

	log.Println(response.StatusCode)
	log.Println(response.Status)
	//getting the response
	data, err 	  := ioutil.ReadAll(response.Body)
	helper.IfError(err, "Can't get the response the URL (ioutil.ReadAll)")
